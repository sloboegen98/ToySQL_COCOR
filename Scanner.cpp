...
void Scanner::Init() {
	...
	keywords.set(L"FROM", 7);
	keywords.set(L"WHERE", 8);
	keywords.set(L"AND", 12);
	keywords.set(L"OR", 13);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 7:
			case_7:
			if ((ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_8;}
			else {goto case_0;}
		case 8:
			case_8:
			recEnd = pos; recKind = 4;
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_8;}
			else {t->kind = 4; break;}
		case 9:
			case_9:
			recEnd = pos; recKind = 3;
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_9;}
